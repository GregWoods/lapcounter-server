...
- `GET /races/pending/` — loads or creates a pending race; when the session has a `races_per_driver` target it selects the next race's drivers via `select_balanced_race_drivers()` (see below) and returns 409 when every active driver has reached the target
...
> **Lifecycle pattern: ending is automatic, starting is manual.** A race ends automatically (target laps / time expiry); the operator manually stages + starts the next (`/racecontrol`: Next Race → Start Race). A session ends automatically (every driver reached `races_per_driver`); the operator starts the next session by starting its first race. lapdata owns the *automatic* side (it POSTs start/finish to the API on transitions); `/racecontrol` owns the *manual* side. `POST /sessions/{id}/finish` likewise ends a session without promoting the next.
...
- **Balanced session-end scheduling** (`select_balanced_race_drivers()` in `next_race.py`): picks the next race's drivers so the session finishes with **every driver having raced exactly `races_per_driver` times, with no tiny final race**. It spreads remaining driver-slots over the fewest races (`ceil(slots / lanes)`) and shrinks later races evenly — e.g. 7 drivers / 6 lanes / target 3 ⇒ race sizes 6,5,5,5 (not 6,6,6,3). Drivers are taken fewest-raced-first (SQL pre-sorts). `compute_session_progress()` projects `session_races_done` / `session_races_total` for the "Race X of Y" title.
...
- Admin session form is simplified: **Race Type** (Finishing Position / Fastest Lap), **Race Laps** *or* **Race Time (min)**, **Points scoring** (Finishing Position only), and **Session ends after N races per driver**. No End Condition / Scoring method / Ranking / Start+End time fields.
