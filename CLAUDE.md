...
**BLE anchors the powerbase's clock to ours rather than using arrival time.** The Slot characteristic reports crossings in ticks since the powerbase's timer was last reset, which has no reporting jitter in it but is not a wall clock. ⚠️ **A tick is 10 ms, not the millisecond the protocol doc says**: measured on a real ARC Pro on 2026-09-18, and read as ms every lap came out ~10x short and was dropped under `MINIMUM_LAP_TIME`. `DEVICE_TICK_S`/`device_seconds()` in `ble_to_timestamps.py` is the one place the unit lives, and `hardware_check.py` uses it too (the mock powerbase has its own `TICK_S` on purpose). That run also measured a **~1.8 s Slot rotation** (one packet per ~300 ms), so a crossing can reach the leaderboard up to ~1.8 s late; see `ble/HARDWARE_VALIDATION.md`. `_device_to_wall()` converts it using a running **minimum** of `(arrival - device_time)`: every sample is the true offset plus some transport delay, delay is never negative, so the smallest sample seen is the best estimate and it converges within a few crossings. The anchor is thrown away and rebuilt whenever a device timestamp moves **backwards** (the powerbase zeroes its timers on Command 0/1), on reconnect (it may have been power-cycled), or when a Command write **restarts timestamps after a halt** (`_note_command_applied()`). ⚠️ That last one is easy to miss: commands 0/2/4 freeze the clock without moving it backwards — and 4 (`POWER_ON_TIMER_HALT`) is what every yellow flag's power cut sends. A D-second halt makes every later sample D bigger, which a running minimum never corrects, so without the re-anchor every post-halt crossing is stamped D seconds early: short laps, or laps dropped under `MINIMUM_LAP_TIME`. The mirror case — **our** clock stepping forward while connected (the Pi's clock set by NTP or by hand after `ble` connected; in dev, the Docker VM's wall clock catching up after the host sleeps while the simulator's monotonic clock stood still) — has the same shape with no command to hook, so `_device_to_wall()` detects it: when every sample for `CLOCK_STEP_CONFIRM_S` (5s) sits more than `CLOCK_STEP_THRESHOLD_S` (3s) above the anchor, it re-anchors to the smallest of them. The threshold must stay above one round-robin **rotation** (one Slot packet per car ID, all 6 in turn — the worst-case reporting delay), and the confirmation stops a BLE stall's late burst from dragging the anchor up; don't drop either. A step under the threshold is never corrected and shortens lap 1 of every later race by that much, which matters at ~5s laps — so 3s is provisional. The first hardware run's ~1.8 s rotation puts ~2× at 3.6 s, so don't lower it without a rotation measured on the hardware the meet runs on. Before this, a dev laptop that slept overnight stamped crossings 31481669ms (~8.7h) behind their arrival and lapdata fell back to arrival time. A constant error in the anchor would cancel out of lap-to-lap deltas anyway; keeping it small also keeps lap 1 honest, since that one is timed from `race_start_time` rather than a previous crossing. ⚠️ **A redesign is planned, not started:** Layer 1 publishes its raw counter plus a clock epoch, and lapdata computes laps from counter differences, so the anchor only matters for lap 1. It also moves the start/first-crossing flag from the meeting to the session. See `docs/lap-timing-clocks.md` (why) and `docs/lap-timing-plan.md` (how, with Greg's decisions) before making non-trivial changes to this anchoring.
...
BLE maps power per state: `handle_race_control()` sends `POWER_ON_TIMER_HALT` immediately on an explicit `pause` command (the no-grace manual stop) and `POWER_ON_RACING` for `prepare`/`arm`/`start`/`yellow`/`resume`/`end`. `handle_race_state()` is the belt-and-braces counterpart (and the *only* path for the Yellow→Paused grace-expiry, per the above) via a small state→command table: `Running`/`Yellow` → `POWER_ON_RACING`, `Paused` → `POWER_ON_TIMER_HALT`. That path is edge-triggered, so on every **BLE connect** `run()` writes `_command_for_race_state(_last_seen_race_state)` rather than a blanket `POWER_ON_RACING` — otherwise a reconnect during `Paused` would restore power with marshals on track — and on **MQTT connect** BLE publishes `race_control` `status`, since `race_state` isn't retained and a restarted container would otherwise not know the race is Paused. **Limp mode (capping power to ~30% instead of a hard stop) is deliberately not implemented** — it still needs the real-hardware tuning sweep described below, and combining it with the grace period is future work, not this pass.
