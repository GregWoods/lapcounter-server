...
The BLE protocol is Scalextric's official doc (`Scalextric_ARC_BLE_Protocol_live-1.docx`, obtained via customerservices.uk@scalextric.com — a copy exists on Greg's machine under `OneDrive\Scalextric ARC Firmware, Protocols etc\`), cross-checked against [RazManager/ScalextricArcBleProtocolExplorer](https://github.com/RazManager/ScalextricArcBleProtocolExplorer). `ble/` only implements the **Slot characteristic** (`0x3B0B`) for lap timing so far. The protocol exposes several other characteristics that GPIO structurally cannot — noted here so they aren't lost before BLE is adopted:
...
**BLE will need to become bidirectional.** `ble/ble_to_timestamps.py` currently only publishes — it never subscribes to MQTT or writes to the powerbase. Two kinds of inbound write need adding, and they're architecturally different:
- **Race lifecycle → Command characteristic writes.** Starting/pausing/ending a race needs to write the Command characteristic (`0x3B0A`) state machine (`POWER_ON_RACING`, `POWER_ON_TIMER_HALT`, `NO_POWER_TIMER_STOPPED`, etc.) on the powerbase — this also resolves the earlier open question of whether Slot timestamps need `POWER_ON_RACING` to tick at all. The natural trigger is the **existing `race_control` topic** — BLE would subscribe to it exactly like LapData already does, translating `start`/`pause`/`resume`/`end` into the matching Command write. No new topic needed; `race_control` is already "any client can publish, any client can subscribe."
- **Refuel.** This one needs a decision, not just plumbing: does it belong in BLE at all? Per the split above, **LapData** owns `fuel_remaining` (it's the one diffing/thresholding `car_throttle`), so a refuel action is naturally just LapData resetting its own in-memory value — no round-trip to BLE required, unless BLE ends up needing to know the fuel level itself (e.g. to drive rumble directly from hardware state rather than being told discrete commands). Recommended default: keep BLE dumb, handle refuel entirely inside LapData, and only route a message to BLE if a future feature (like rumble-on-low-fuel) needs BLE to *act* on it.

Implementation note for whenever this is built: `ble_to_timestamps.py` runs bleak's async GATT client in the main thread's asyncio loop, but MQTT is the synchronous `paho` client running its own background thread (`loop_start()`). An inbound MQTT message that needs to trigger a GATT write can't call `await client.write_gatt_char(...)` directly from paho's `on_message` callback — it has to hand off across threads, e.g. `asyncio.run_coroutine_threadsafe(client.write_gatt_char(...), loop)`.

None of this is implemented — `ble/` currently only does lap timing, to keep parity with GPIO while that swap gets validated on real hardware first.
