...
| **E12** Physical power cycle mid-race | ⚠️ **Press Yellow Flag or Pause first** (see the warning below), then switch the powerbase off at the mains mid-race, wait ~30s as if clearing a track short, switch it back on and resume. Drive 3 more laps. | No phantom laps on reconnect (seeding). `ble` logs a **new clock** at the reconnect, so no lap is timed across the reset. The lap spanning the outage is long — roughly the whole outage — and its `driver_lap` says `"timing":"anchored"` (or `"arrival"` if its packet beat the first heartbeat on the new clock). ⚠️ **No lap time is ever negative or absurdly short**: the powerbase zeroes its timers, so the counter after the cycle is smaller than the one before it, and only the clock change stops that becoming a negative subtraction. Laps after the resume are `counter` again and sane. Lap counts continue from where they were. |
